...
SELECT c.name 
FROM countries AS c 
JOIN countrylanguages AS cl ON c.code = cl.countrycode
...
SELECT ct.name 
FROM cities AS ct
JOIN countries AS c ON c.code = ct.countrycode
WHERE c.code = 'SMR'
AND ct.name != c.name;
...
AND LOWER(ct.name) LIKE 'serra%';

SELECT c.name 
FROM countries AS c
JOIN cities AS ct ON c.code = ct.countrycode
